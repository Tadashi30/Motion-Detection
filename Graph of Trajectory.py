...
while cap.isOpened():
    ...
    key = cv2.waitKey(1)
    if key == 27:  # 'q' key or 'Escape' key
        break
    # Plot centroid trajectory
    if len(centroid_path) > 1:
        # Separate x and y coordinates of centroid_path
        x_coords = [coord[0] for coord in centroid_path]
        y_coords = [coord[1] for coord in centroid_path]
